...
3. **Work through one phase at a time** — don't skip ahead

### Phase order:

```
renderer/README.md → training/README.md → generator/README.md → viewer/README.md
```

Each phase has its own README with specific instructions. Claude Code can help you build out the actual implementation code in each directory — the repo structure is set up as a scaffold for you to fill in.
...
- The renderer and viewer are web-based (HTML/JS) — use VS Code's Live Server extension to preview
...
This is flexible — the actual development time depends on how much iteration you need on the training pairs and edge case fixes.
